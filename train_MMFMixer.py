...
from models.model_fuse import MMFMixer, ImgNet, SVNet
import torchvision.models as models

# Experiments on Shenzhen City.
...
def main(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        start = time.time()
        index = 0
        for images, sv, labels in train_loader:
            ...
            img_feature, outputs2 = model2(images, sv)
        ...
        if epoch % 1 == 0:
            ...
            total_all = 0
            with torch.no_grad():
                for images, sv, labels in val_loader:
                    images = images.to(device)
                    # print(images.shape)
                    labels = labels.to(device)
                    sv = sv.to(device)
        ...
        if acc_2 > best_acc_2:
            ...
            torch.save(model2, os.path.join(args.model_path, 'MMF-2-2-6-1000.pth'))
            best_acc_2 = acc_2
            best_epoch = epoch
        # if acc_3 > best_acc_3:
